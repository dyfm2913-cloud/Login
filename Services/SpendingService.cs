...
namespace Services
{
    public class SpendingService : ISpendingService
    {
        ...
        public List<SpendingView> GetAllSpending()
        {
            ...
            try
            {
                using (var connection = _databaseService.GetConnection(Common.Enums.DatabaseType.AppDatabase))
                {
                    connection.Open();
                ...
                Logger.LogInfo($"تم جلب {spendingList.Count} سند صرف بنجاح");
            }
            catch (Exception ex)
            {
                Logger.LogError($"خطأ في جلب جميع سندات الصرف: {ex.Message}", ex);
        ...
        public List<SpendingView> GetSpendingByDate(DateTime date)
        {
            ...
            try
            {
                ...
                Logger.LogInfo($"تم جلب {spendingList.Count} سند صرف للتاريخ {date:yyyy/MM/dd}");
            }
            catch (Exception ex)
            {
                Logger.LogError($"خطأ في جلب سندات الصرف للتاريخ {date:yyyy/MM/dd}: {ex.Message}", ex);
        ...
        public List<SpendingView> GetSpendingByDateRange(DateTime startDate, DateTime endDate)
        {
            ...
            try
            {
                ...
                Logger.LogInfo($"تم جلب {spendingList.Count} سند صرف للفترة {startDate:yyyy/MM/dd} - {endDate:yyyy/MM/dd}");
            }
            catch (Exception ex)
            {
                Logger.LogError($"خطأ في جلب سندات الصرف للفترة {startDate:yyyy/MM/dd} - {endDate:yyyy/MM/dd}: {ex.Message}", ex);
        ...
        public SpendingView GetSpendingById(long id)
        {
                ...
                using (var connection = _databaseService.GetConnection(Common.Enums.DatabaseType.AppDatabase))
                {
                    ...
                    using (var command = new SqlCommand("SELECT * FROM vewSpending WHERE ID = @ID", connection))
                    {
                        ...
                        using (var reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                Logger.LogInfo($"تم جلب سند الصرف بالمعرف {id}");
            ...
            {
                Logger.LogError($"خطأ في جلب سند الصرف بالمعرف {id}: {ex.Message}", ex);
        ...
        public bool AddSpending(Spending spending)
        {
            try
            {
                using (var connection = _databaseService.GetConnection(Common.Enums.DatabaseType.AppDatabase))
                {
                        ...
                        connection))
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            Logger.LogInfo($"تم إضافة سند صرف جديد رقم {spending.TheNumber}");
                            return true;
                        }
                    }
                }
            ...
            {
                Logger.LogError($"خطأ في إضافة سند الصرف: {ex.Message}", ex);
        ...
        public bool UpdateSpending(Spending spending)
        {
            try
            {
                using (var connection = _databaseService.GetConnection(Common.Enums.DatabaseType.AppDatabase))
                {
                        ...
                        connection))
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            Logger.LogInfo($"تم تحديث سند الصرف رقم {spending.TheNumber}");
                            return true;
                        }
                    }
                }
            ...
            {
                Logger.LogError($"خطأ في تحديث سند الصرف {spending.ID}: {ex.Message}", ex);
        ...
        public bool DeleteSpending(long spendingID)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            using (var command = new SqlCommand("DELETE FROM tblSpending WHERE ID = @ID", connection, transaction))
                            {
                                ...
                                if (rowsAffected > 0)
                                {
                                    Logger.LogInfo($"تم حذف سند الصرف {spendingID}");
                                    return true;
                                }
                            }
                        }
            ...
            {
                Logger.LogError($"خطأ في حذف سند الصرف {spendingID}: {ex.Message}", ex);
                throw new Exception($"خطأ في حذف سند الصرف {spendingID}: {ex.Message}", ex);
            }
        ...
        public long GetNextVoucherNumber()
        {
            ...
            {
                Logger.LogError($"خطأ في جلب الرقم التالي للسند: {ex.Message}", ex);
                return 1;
            }
        }
        ...
        public List<SpendingView> SearchSpending(string searchTerm, DateTime? fromDate, DateTime? toDate)
        {
            var spendingList = new List<SpendingView>();
            ...
            try
            {
                using (var connection = _databaseService.GetConnection(Common.Enums.DatabaseType.AppDatabase))
                {
                    ...
                    string query = "SELECT * FROM vewSpending WHERE 1=1";
                    var parameters = new List<SqlParameter>();

                    if (!string.IsNullOrWhiteSpace(searchTerm))
                    {
                        query += " AND (ملاحظات LIKE @SearchTerm OR رقم المرجع LIKE @SearchTerm OR المستخدم LIKE @SearchTerm)";
                        parameters.Add(new SqlParameter("@SearchTerm", $"%{searchTerm}%"));
                    }

                    if (fromDate.HasValue)
                    {
                        query += " AND CONVERT(DATE, التاريخ) >= @FromDate";
                        parameters.Add(new SqlParameter("@FromDate", fromDate.Value.Date));
                    }

                    if (toDate.HasValue)
                    {
                        query += " AND CONVERT(DATE, التاريخ) <= @ToDate";
                        parameters.Add(new SqlParameter("@ToDate", toDate.Value.Date));
                    }

                    query += " ORDER BY التاريخ DESC, الرقم DESC";
                ...
                Logger.LogInfo($"تم العثور على {spendingList.Count} سند صرف بنتيجة البحث");
            }
            catch (Exception ex)
            {
                Logger.LogError($"خطأ في البحث عن سندات الصرف: {ex.Message}", ex);
                throw new Exception($"خطأ في البحث عن سندات الصرف: {ex.Message}", ex);
            }
