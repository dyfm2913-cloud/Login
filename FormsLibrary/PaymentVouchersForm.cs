...
namespace FormsLibrary
{
    public partial class PaymentVouchersForm : Form
    {
        ...
        {
            dataGridViewSpending.Rows.Insert(0,
                null, GetNextVoucherNumber(), DateTime.Today.ToString("yyyy/MM/dd"),
                "نقدي", 0, "دينار", 1, 0, "دينار", "", "", 1,
                _currentUser?.FullName ?? "المستخدم الحالي", "", ""
            );
        }
        ...
        private void SaveNewSpending()
        {
            ...
            {
                if (dataGridViewSpending.Rows.Count == 0 ||
                    dataGridViewSpending.Rows[0].Cells["المبلغ"].Value == null ||
                    Convert.ToDecimal(dataGridViewSpending.Rows[0].Cells["المبلغ"].Value) <= 0)
                {
                ...
                {
                    TheNumber = Convert.ToInt64(dataGridViewSpending.Rows[0].Cells["الرقم"].Value),
                    TheDate = DateTime.Parse(dataGridViewSpending.Rows[0].Cells["التاريخ"].Value.ToString()),
                    TheMethod = GetPaymentMethodId(dataGridViewSpending.Rows[0].Cells["طريقة_الصرف"].Value?.ToString()),
                    Amount = Convert.ToDecimal(dataGridViewSpending.Rows[0].Cells["المبلغ"].Value),
                    CurrencyID = GetCurrencyId(dataGridViewSpending.Rows[0].Cells["العملة"].Value?.ToString()),
                    AccountID = dataGridViewSpending.Rows[0].Cells["الصندوق"].Value != null ? 
                                Convert.ToInt64(dataGridViewSpending.Rows[0].Cells["الصندوق"].Value) : 1L,
                    Notes = dataGridViewSpending.Rows[0].Cells["ملاحظات"].Value?.ToString() ?? "",
